...
async function scoreResourcesWithML(
  ...
  const predictionPromises = resources.map(async (resource) => {
    const payload: MLPredictionRequest = {
      ...
      target_grade: resource.target_grade || userProfile.grade_level || 'Class 9',
    };
...
function fallbackSort(
  ...
  return resources.sort((a, b) => {
    ...
    const aDiff = difficultyOrder[a.difficulty as keyof typeof difficultyOrder] || 999;
    const bDiff = difficultyOrder[b.difficulty as keyof typeof difficultyOrder] || 999;
